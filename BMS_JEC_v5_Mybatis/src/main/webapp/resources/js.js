...
function confirmId(){
	...
	var url = "member_confirmid?id="+document.inputform.id.value;
...
//장바구니 수량 수정된 값 넘기기
function cart_quancon( c_num , quan , b_num ){
	...
	window.location="cart_quancon?c_num="+c_num+"&quan="+quan+"&b_num="+b_num;
...
function cart_reqorder( c_num, quan , b_num){
	...
	window.location="cart_reqorder?c_num="+c_num+"&quan="+quan+"&b_num="+b_num;
...
//장바구니 수량 수정된 값 넘기기
function cart_cartdelete( c_num  ){
	/*alert(c_num);*/
	window.location="cart_cartdelete?c_num="+c_num;
}
...
function request(state){
	...
	if( state == "CART" ){
		document.maindetailform.action="cart_listview?state=CART";
	}
	
	if( state == "ORDER" ){
		document.maindetailform.action="order_view?state=ORDER";
