		...
		<tr><th colspan="8" align="right" style="hight:25px;" >
			...
			<c:choose>		
				...
				<c:when test="${memkind==1 }">
					<a href="book_writeform"> 도서추가 </a> &emsp;
				</c:when>
		...
		<c:if test="${cnt>0}">
			...
			<tr>
				...
				<td align="left" style="margin-left: 20px;"> 
					...
					<a href="book_detail?b_num=${d.b_num}&pageNum=${pageNum}&number=${number+1}">
						${d.title}</a>
				</td>	
			...
			<tr>
				...
				<th colspan="8">	<!-- 화살표 및 페이지 번호 -->
					...
					<c:if test="${startPage > pageBlock}">	
						<a href="book_list"> [◀◀] </a>	
						<a href="book_list?pageNum=${startPage-pageBlock}"> [◀] </a>	
					</c:if>
					...
					<c:if test="${pageCount > endPage}">
						<a href="book_list?pageNum=${startPage + pageBlock}"> [▶] </a>	
						<a href="book_list?pageNum=${pageCount}"> [▶▶]</a>		
					</c:if>
