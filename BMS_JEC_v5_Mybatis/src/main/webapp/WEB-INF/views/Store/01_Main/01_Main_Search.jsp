...
<body onload="search_focus()">
	...
	<% System.out.println(" -> 01_Main_Search View "); %>
	...
	<form action="main_search" name="searchform" onclick="return srarch_Check()" method="post">	
		
		<table align="center" style="width:100%">
			<tr style="width:85%" align="center">
				<td> 
					<input name="str" type="text" value="수학" "${str}"  style="width:50%";  > 
					&ensp;
		...
		<c:if test="${cnt>0}">
			...
			<tr style=" text-align:center; height: 1em; text-overflow: ellipsis; 
				...
				<td align="left" style="margin-left: 20px;"> 
					...
					<a href="main_detail?b_num=${book.b_num}&pageNum=${pageNum}&start=${start}&str=${str}"> ${book.title} </a>	
			...
			<tr>
				...
				<th colspan="8">	<!-- 화살표 및 페이지 번호 -->
					...
					<c:if test="${startPage > pageBlock}">	
						<a href="main_serch?str=${str}"> [◀◀] </a>	
						<a href="main_serch?str=${str}&pageNum=${startPage-pageBlock}"> [◀] </a>	
					</c:if>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						...
						<c:if test="${i != currentPage}">
							<a href="main_serch?str=${str}&pageNum=${i}">[${i}]</a>	
						</c:if>
					...
					<c:if test="${pageCount > endPage}">
						<a href="main_serch?str=${str}&pageNum=${startPage + pageBlock}"> [▶] </a>	
						<a href="main_serch?str=${str}&pageNum=${pageCount}"> [▶▶]</a>		
					</c:if>
