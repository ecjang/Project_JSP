				...
				<td>
					...
					<c:if test="${memkind != 1 }">
						<c:if test="${order.state == 'ORDER_COMPLETE' }">
							 <a href="order_statecon?o_num=${order.o_num}&state=ORDER_CANCEL&&pageNum=${pageNum}">
					 ...
					 <c:if test ="${memkind == 1 }">
						<c:if test="${order.state == 'ORDER'}">
							<a href="order_statecon?o_num=${order.o_num}&state=ORDER_COMPLETE&&pageNum=${pageNum}" >
						...
						<c:if test="${order.state == 'ORDER_CANCEL' }">
						 <a href="order_statecon?o_num=${order.o_num}&state=SALE&&pageNum=${pageNum}">  
			...
			<tr>
				...
				<th colspan="9">	
					...
					<c:if test="${startPage > pageBlock}">	
						<a href="order_listpro"> [◀◀] </a>	
						<a href="order_listpro?pageNum=${startPage-pageBlock}"> [◀] </a>	
					</c:if>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						...
						<c:if test="${i != currentPage}">
							<a href="order_listpro?&pageNum=${i}">[${i}]</a>	
						</c:if>
					...
					<c:if test="${pageCount > endPage}">
						<a href="order_listpro?pageNum=${startPage + pageBlock}"> [▶] </a>	
						<a href="order_listpro?pageNum=${pageCount}"> [▶▶]</a>		
					</c:if>
