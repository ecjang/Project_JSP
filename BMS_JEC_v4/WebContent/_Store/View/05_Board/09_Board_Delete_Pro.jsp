	...
	<c:if test="${scnt==1}">	<!-- 게시물이 있는 경우 -->
		...
		<c:if test="${dcnt==1}">	<!-- 답변이 없는 경우  -->
			<script type="text/javascript">
				...
				window.location="17_board_list.do?kind=${kind}&pageNum=${pageNum}";
		...
		<c:if test="${dcnt==-1}">	<!-- 답변이 있는 경우 -->
			<script type="text/javascript">
				...
				window.location="18_board_list.do?kind=${kind}&pageNum=${pageNum}";
			</script>
		</c:if>
		...
		<c:if test="${dcnt!=0}">	<!-- 답변이 없는 경우  -->
			<script type="text/javascript">
				alert("정상적으로 삭제 되었습니다.");	
				window.location="19_board_list.do?kind=${kind}&pageNum=${pageNum}";
